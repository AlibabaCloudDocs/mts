...
截图配置 [AliyunSnapshotConfig](#section_m5h_xwh_y2b)
...
|Id|String|作业ID。|
|Input|AliyunOSSFile|作业输入。|
|CoverConfig|AliyunCoverConfig|智能封面配置。|
|PipelineId|String|管道ID。|
|State|String|作业状态，Submitted、Analyzing、Success、Fail。|
|Code|String|作业失败时错误码。|
|Message|String|作业失败时错误信息。|
|UserData|String|用户自定义数据。|
|CoverImageList|AliyunCoverImage\[\]|智能封面结果，最多显示5张，如果没有列表为空。|
...
|:-|:-|:-|
|Url|String|智能封面url。|
|Score|String|取值范围\[0, 1\]，精度小数点后5位，截图图片对视频表述性分值。|
|Time|String|图片截取时间点。|
