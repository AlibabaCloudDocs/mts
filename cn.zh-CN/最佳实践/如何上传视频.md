...
2.  在**概览**页，将光标向下滑动在**大家都在用**模块中，单击**安全令牌** \> **前往RAM控制台**。
3.  进入**安全令牌快捷配置**页面，单击**开始授权**。
4.  系统进行自动授权，请您保存AccessKey ID、AccessKey Secret、RoleArn参数信息。单击**保存AK信息**，对话框会关闭，STS的开通完成。
